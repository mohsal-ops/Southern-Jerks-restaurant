...
import TrafficSourceChart from "@/app/admin/_components/charts/trafficSources";

const isUnlocked = false; // 🔒 flip to true when ready to show Jordan
...
function getHoustonStatus() {
  ...
  if (!today.open)
    return { isOpen: false, label: "Closed today", next: "Opens Tuesday 11:00 AM" };

  if (hour >= today.open && hour < today.close!) {
    const h = today.close!;
    const display = h > 12 ? `${h - 12}:00 PM` : `${h}:00 AM`;
    return { isOpen: true, label: "Open now", next: `Closes at ${display}` };
  }

  const display =
    today.open > 12 ? `${today.open - 12}:00 PM` : `${today.open}:00 AM`;
  return { isOpen: false, label: "Currently closed", next: `Opens at ${display}` };
}
...
async function getDashboardData() {
  ...
  if (latestPost) {
    const daysSince = Math.floor(
      (Date.now() - new Date(latestPost.createdAt).getTime()) / (1000 * 60 * 60 * 24)
    );
    if (daysSince <= 7) seoScore += 10;
    else if (daysSince <= 30) seoScore += 5;
  }

  return {
    totalItems, activeItems, inactiveItems, totalCategories,
    totalPosts, latestPost, totalLocations, featuredItems, seoScore,
  };
}
...
function SEOTip({ done, text, action, href }: {
  done: boolean; text: string; action?: string; href?: string;
}) {
  return (
    <div className="flex items-start gap-3 py-3 border-b border-stone-50 last:border-0">
      ...
      {!done && href && action && (
        <Link href={href} className="text-xs font-semibold text-[#c85a1e] hover:underline flex-shrink-0">
          {action} →
        </Link>
      )}
    </div>
...
export default async function Page() {
  ...
  const daysSincePost = data.latestPost
    ? Math.floor((Date.now() - new Date(data.latestPost.createdAt).getTime()) / (1000 * 60 * 60 * 24))
    : null;
        ...
        <div className="min-h-screen bg-stone-50 p-6 space-y-8">
            ...
            <div className="bg-amber-50 border border-amber-200 rounded-2xl p-4 flex items-center gap-3">
              ...
              <div className="flex-1">
                <p className="text-sm font-semibold text-amber-800">
                  {data.inactiveItems} menu item{data.inactiveItems > 1 ? "s" : ""} currently unavailable
                </p>
                <p className="text-xs text-amber-600 mt-0.5">Customers can see these but can't order them</p>
              </div>
              <Link href="/admin/menuItems" className="text-xs font-semibold text-amber-700 flex items-center gap-1">
          ...
          <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              <Link href="/admin/Blog" className="group bg-white rounded-2xl border border-stone-200 shadow-sm p-5 hover:border-[#c85a1e] hover:shadow-md transition-all">
                <div className="w-9 h-9 rounded-xl bg-orange-50 text-[#c85a1e] flex items-center justify-center mb-3 group-hover:scale-110 transition-transform">
                  <PenLine size={18} />
                </div>
                <p className="font-semibold text-stone-800 text-sm">Write Blog Post</p>
                <p className="text-xs text-stone-400 mt-1">
                  {daysSincePost === null ? "No posts yet — start now" : daysSincePost === 0 ? "Last post: today ✓" : `Last post: ${daysSincePost}d ago`}
                </p>
              </Link>
              <Link href="/admin/menuItems/new" className="group bg-white rounded-2xl border border-stone-200 shadow-sm p-5 hover:border-[#1a6b3c] hover:shadow-md transition-all">
                <div className="w-9 h-9 rounded-xl bg-green-50 text-[#1a6b3c] flex items-center justify-center mb-3 group-hover:scale-110 transition-transform">
              ...
              <Link href="/admin/places" className="group bg-white rounded-2xl border border-stone-200 shadow-sm p-5 hover:border-[#1d4ed8] hover:shadow-md transition-all">
                <div className="w-9 h-9 rounded-xl bg-blue-50 text-[#1d4ed8] flex items-center justify-center mb-3 group-hover:scale-110 transition-transform">
                  <MapPin size={18} />
                </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
              <StatCard label="Blog Posts" value={data.totalPosts}
                sub={data.latestPost ? `Last: "${data.latestPost.title.slice(0, 22)}..."` : "No posts yet"}
                icon={<BookOpen size={16} />} accent="#c85a1e" />
              <StatCard label="Active Menu Items" value={data.activeItems}
                sub={`${data.inactiveItems} unavailable`} icon={<UtensilsCrossed size={16} />} accent="#1a6b3c" />
              <StatCard label="Featured Items" value={data.featuredItems}
                sub="Shown on home page" icon={<Star size={16} />} accent="#d97706" />
              <StatCard label="Locations" value={data.totalLocations}
                sub="On the map" icon={<MapPin size={16} />} accent="#1d4ed8" />
            </div>
          </div>
                ...
                <div className="relative w-16 h-16 flex-shrink-0">
                  <svg viewBox="0 0 36 36" className="w-16 h-16 -rotate-90">
                    ...
                    <circle cx="18" cy="18" r="15.9" fill="none" stroke={seoColor} strokeWidth="3"
                      strokeDasharray={`${data.seoScore} 100`} strokeLinecap="round" />
                  </svg>
            ...
            <div className="bg-white rounded-2xl border border-stone-200 shadow-sm p-6">
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-3">
                  <div className="w-9 h-9 rounded-xl bg-orange-50 text-[#c85a1e] flex items-center justify-center">
                    <TrendingUp size={16} />
                  </div>
                  <div>
                    <p className="font-semibold text-stone-800 text-sm">
                      {data.totalPosts} post{data.totalPosts !== 1 ? "s" : ""} published
                    </p>
                    <p className="text-xs text-stone-400">
                      {data.latestPost
                        ? `Last: "${data.latestPost.title}" — ${daysSincePost === 0 ? "today" : `${daysSincePost} days ago`}`
                        : "No posts yet — start writing to get found on Google"}
                    </p>
                  </div>
                </div>
                <Link href="/admin/Blog" className="text-xs font-semibold text-[#c85a1e] hover:underline flex items-center gap-1">
              ...
              <div className="px-6 pb-5">
                <div className="w-full h-2 bg-stone-100 rounded-full overflow-hidden">
                  <div className="h-full bg-green-500 rounded-full transition-all"
                    style={{ width: `${data.totalItems > 0 ? (data.activeItems / data.totalItems) * 100 : 0}%` }} />
                </div>
                <p className="text-xs text-stone-400 mt-2">
                  {data.totalItems > 0
                    ? `${Math.round((data.activeItems / data.totalItems) * 100)}% of menu currently available`
                    : "No items yet"}
                </p>
              </div>
            </div>
